...
def configurar_php():
    if os.system("echo " + config['sudo']['password'] + " | sudo chmod +x php.sh && sudo ./php.sh " + config['PHP']['dir'] + " " + config['PHP']['allow'] + " " + config['PHP']['session']) == 0:
        return True
    else:
...
def migracion():
    if os.system("echo " + config['sudo']['password'] + " | sudo chmod +x migracion.sh && sudo ./migracion.sh " + config['PostgreSQL']['database'] + " " + config['PostgreSQL']['user'] + " " + config['PostgreSQL']['password'] + " " + config['PostgreSQL']['host'] + " " + config['PostgreSQL']['port']) == 0:
        return True
    else:
